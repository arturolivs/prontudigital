...
- [ ] **Upload de anexo e avatar falha** — `/dados/anexos` não existe na imagem, o volume nasce `root:root` e o backend roda como `prontu`. Sobe normal e só quebra no primeiro upload. → `backend/Dockerfile.prod`: `RUN mkdir -p /dados/anexos && chown -R prontu:prontu /dados` antes do `USER`.
- [ ] **Backend sem saída para a internet** — está só na rede `interna` (`internal: true`), sem DNS nem egress; `graph.facebook.com` é inalcançável, então o WhatsApp não vai funcionar quando for ligado, ao contrário do que o `.env.prod.example` promete. → adicionar `externa` às `networks` do backend (sem `ports:` continua inalcançável de fora).
...
- [ ] `-Djava.security.egd=file:/dev/./urandom` obsoleto desde o JDK 9 (`backend/Dockerfile.prod:43`).
